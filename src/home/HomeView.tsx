...
export type Domain = 'verbs' | 'counters';
  ...
  return (
    <div className="flex flex-col items-center gap-4 px-6 py-12 text-center">
      <h1 className="text-4xl font-medium tracking-tight text-neutral-950 dark:text-neutral-100">
        {domain === 'verbs' ? '日本語動詞練習' : '助数詞練習'}
      </h1>
      <p className="mb-2 text-neutral-500 dark:text-neutral-400">
        {domain === 'verbs' ? 'N5/N4 verb conjugation practice.' : 'Learn which counter word goes with which object.'}
      </p>
      ...
      <div className="flex gap-2 rounded-full bg-neutral-100 p-1 dark:bg-neutral-800" role="radiogroup" aria-label="Practice domain">
        <button
          ...
          className={`rounded-full px-5 py-2.5 text-[15px] transition-colors ${
            domain === 'verbs' ? 'bg-purple-600 text-white' : 'text-neutral-500 dark:text-neutral-400'
          }`}
          role="radio"
          aria-checked={domain === 'verbs'}
          onClick={() => onDomainChange('verbs')}
        >
          Verbs
        </button>
        <button
          type="button"
          className={`rounded-full px-5 py-2.5 text-[15px] transition-colors ${
            domain === 'counters' ? 'bg-purple-600 text-white' : 'text-neutral-500 dark:text-neutral-400'
          }`}
          role="radio"
          aria-checked={domain === 'counters'}
          onClick={() => onDomainChange('counters')}
        >
          Counters
        </button>
      </div>
        ...
        <div className="mt-2 w-full max-w-[480px]">
          ...
          <div className="grid grid-cols-3 gap-2" role="group" aria-label="Counters to practice">
            {counters.map((counter) => {
              const active = selectedCounterIds.includes(counter.id);
              return (
                <button
                  key={counter.id}
                  type="button"
